...
calc_burst_distributions <- function(s, min_vals=1, xlimit=25, bins_in_sec=5,
  ...
  #   bins_in_sec:  how many bins to cut each of the segments. For example: IBI analysis has 25 seconds as xlimit,
  #     to analyse in a 0.1 sec resolution bins_in_sec should be set to 10, for 1 sec resolution set bins_in_sec to 1
  #   feature  :  what feature to analyze, options are "IBI", "ISI, "nspikesInBurst", "duration", "spikesDensityInBurst"
  #   filter_values_by_min:  should analysis disregard values with lower then filter_values_by_min number of values ? (0/1, default is 0)
  #     for example, if set to 1 for duration analysis, should analysis consider also bursts shorter than filter_values_by_min ?
  ...
  write(paste("->->-> Analysing ", s$file, sep = ""), file = log_file, append = TRUE)

  cat(paste("Arguments: min_vals=", min_vals, "; xlimit=", xlimit, "; bins_in_sec=",
    bins_in_sec, ";per_well=", per_well, "; duration=", duration, "; feature='", feature, "'; filter_values_by_min=", filter_values_by_min, "; min_values=", min_values, "\n", sep = "")) # 
  ...
  fVals <- NULL
  if (feature == "IBI") {
    fVals <- .calc_all_ibi(s, s$allb)
  } else if (feature == "ISI") {
    fVals <- .calc_all_isi(s, s$allb)
  } else if (feature == "nspikesInBurst") {
    for (j in 1:length(s$channels)) {
      ## Number of spikes in burst !
      fVals[j] <- get_burst_info(s$allb[j], "len")
    }} else if (feature == "duration") {
    for (j in 1:length(s$channels)) {
      ## Duration of burst
      fVals[j] <- get_burst_info(s$allb[j], "durn")
    ...
    for (j in 1:length(s$channels)) {
      if (length(get_burst_info(s$allb[j], "durn")[[1]]) > 1) {
        ...
        fVals[j] <- mapply("/", get_burst_info(s$allb[j], "len"), get_burst_info(s$allb[j], "durn"), SIMPLIFY = FALSE)
      } else {
        fVals[j] <- 0
  ...
  if (length(wells) > 0) {
    for (well in wells) {
      ...
      if (treat == "") {
        write(paste("Skipping well-", well, " that has active electrodes since treatment property is empty.", sep = ""), file = log_file, append = TRUE)
      }
      active_electrodes <- which(s$cw == well & as.vector(unlist(lapply(s$isis, length))) > 0)
      Cper_well <- rbind(Cper_well, data.frame(well, new_count = length(active_electrodes)))
    }} else {
  ...
  #  post=c((1/jump) * (min_values * jump):(duration * jump))
  ...
  all_pos <- rep(posT, times = length(treatments))
  all_trt <- rep(treatments, each = length(posT))
  ...
  for (j in 1:length(s$channels)) {
    ...
    #    print(paste("well-", icurrentwell, " num of ele-", (CpserWell$new_count[Cper_well$well==well])))
    ...
    if (Cper_well$new_count[Cper_well$well == well] < min_electrodes){
      write(paste("Skipped well- ", well, " less than ", min_electrodes, " electrodes", sep = ""), file = log_file, append = TRUE)
      next} else {
      data <- fVals[[j]]
    }
    ...
    if (length(data) > min_vals) {
      e <- hist(data, plot = FALSE, breaks = c((1 / jump) * (min_values * jump):(xlimit * jump)))

      # normalize to number of values in electrode
      hist_data <- data.frame(normHist = (e$counts / length(data)), pos = e$mids, treat = treat)
      ...
      if (per_well == 0) 
      {
        ...
        if (treat %in% treatments) {
          ...
          all[all$treat == treat & all$pos == as.character(hist_data$pos), names(hist_data)[1]] = hist_data[, 1]
        }
      } # END per_well ==0
              ...
              if (current_enum >= min_electrodes)
                {
                  ...
                  rmeans <- apply(well_data[, 1:current_enum], 1, mean)} else {
                write(paste("Less than four electrodes passed filters for ", old, ", it will be removed from the analysis."), file = log_file, append = TRUE)
                valid <- 0
              }
              # Continue if treatment is valid
              if (unique(well_data$treat) %in% treatments && valid == 1) {
                # make a new data frame from the well data
                meansDF <- data.frame(rmeans, pos = well_data$pos, treat = unique(well_data$treat))
                names(meansDF)[1] <- old # change to name of the well that was just analyzed

                # Fill in the matrix with well data
                all[, old] <- NA
                all$pos <- as.character(all$pos)
                all[as.character(all$treat) == as.character(unique(well_data$treat)) & all$pos == as.character(meansDF$pos), old] = meansDF[, 1]
              }} # more than 0 electrodes
          }
          write(paste("Analyzing well ", well, ",  channel number is:", j, sep = ""), file = log_file, append = TRUE)
          old <- well
          current_enum <- 1
          well_data <- hist_data
        } else { # well == old
          if (current_enum == 0){
            well_data <- hist_data} else {
            well_data <- cbind(hist_data[, 1], well_data)}
          current_enum <- current_enum + 1}} else { write(paste("Analysis per electrode, electrode-", icurrentwell, "Analyzed."), file = log_file, append = TRUE)}
    } #    if (length(data) > min_vals){
  ...
  if (per_well == 1){
    ...
    write("Collapsing genotype data by electrode.", file = log_file, append = TRUE)
  } # per_well ?

  if (is.null(table)) 
  {
    write(paste("Based on current parameters there are no cases to plot,  skipping file ", s$file, sep = ""), file = log_file, append = TRUE)
    return(1)
  }
  if ((dim(table)[2] - (first_data_column - 1)) <= 1) 
  {
    write(paste("Not enough electrodes (<=1) to plot, skipping file ", s$file, sep = ""), file = log_file, append = TRUE)
    return(1)
  }
  write(paste("Number of electrodes in table : ", (dim(table)[2] - (first_data_column - 1)), sep = ""), file = log_file, append = TRUE)

  # set the ylimit on the top 2% values of the left most dist values (1st fifth 0 - 0.2 (xlimit / 5))
  lim <- apply(table[, (first_data_column:dim(table)[2])], 1, mean, na.rm = TRUE)
  ...
  for (t in treatments) {
    if (nrow(table[table$treat == t, ]) == 0) {
      ...
      write(paste("Treatment", t, "has no electrodes to contribute data after filters and is excluded from analysis.", sep = " "), file = log_file, append = TRUE)
      next}
    ...
    if (ncol(clean_trt) < 7) # 3 are treat, pos, id + 4 electrodes 
    {
      good_treatments <- good_treatments[good_treatments != t]
      write(paste("Treatment", t, "has less than 4 electrodes to contribute data after filters and is excluded from analysis.", sep = " "), file = log_file, append = TRUE)
      next}
  }
  all_distributions <- NULL
  if (length(good_treatments) <= 1) 
  {
    write(paste("Not enough treatments (<=1) with enough data to plot, skipping file ", s$file, sep = ""), file = log_file, append = TRUE)
    return(1)
  }
  ...
  for (tr in 1:length(good_treatments)) {
    ...
    transposed <- as.data.frame(t(clean_trt[1:ceiling(xlimit * jump), first_data_column:length(clean_trt)]))
    ...
    gmeans <- (apply(tto_plot[, (first_data_column:dim(tto_plot)[2])], 1, mean, na.rm = TRUE))
    ...
    pos <- table$pos
    if (first){
      suppressMessages(gmeans_table <- data.frame(treat = as.character(t), data = gmeans))
      bottom <- factorial(length(good_treatments)) / (2 * (factorial(length(good_treatments) - 2))) + 3
  ...
  legend(xlimit * 0.6, ylimit * 0.8, legend = c(good_treatments), lty = 1, lwd = 5, col = colors, bg = "white", cex = 0.9, y.intersp = 0.7)
  # print values of all ks tests
  line <- 3
  if (length(good_treatments) > 1) {
    for (t2 in 1:(length(good_treatments) - 1)) {
      if (is.na(sum(gmeans_table[gmeans_table$treat == good_treatments[t2], "data"][1:(xlimit * jump)], na.rm = T))) {
        write(paste("Treatment ", t2, " is empty for this recording.", sep = ""), file = log_file, append = TRUE)
        next
      }
      for (t3 in (t2 + 1):length(good_treatments)) {
        if (is.na(sum(gmeans_table[gmeans_table$treat == good_treatments[t3], "data"][1:(xlimit * jump)], na.rm = T))) {
          write(paste("Treatment ", t3, " is empty for this recording.", sep = ""), file = log_file, append = TRUE)
          next
        }

        w <- ks.test(gmeans_table[gmeans_table$treat == good_treatments[t2], "data"][1:(xlimit * jump)], gmeans_table[gmeans_table$treat == good_treatments[t3], "data"][1:(xlimit * jump)], alternative = "two.sided")
        write(paste("Kolmogorov-Smirnov test p.value for treatments ", good_treatments[t2], " vs. ", good_treatments[t3], " : ", format(w$p.value, digits = 2), ", for: ", feature, " max ", xlimit, " seconds", sep = ""), file = log_file, append = TRUE)
        mtext(side = 1, at = 0, line = line,
          text = paste("K-S test for ", good_treatments[t2], " vs. ", good_treatments[t3], " : ", format(w$p.value, digits = 2), ",  for: ", feature, sep = ""), col = "black", cex = 0.9, adj = 0)
        line <- line + 1}}}
  # write all distributions for a permutation test
  table_path <- paste(outputdir, "/", basename, "_", feature, "_distributions.csv", sep = "")
  csvwell <- paste(outputdir, "/", get_project_plate_name(s$file), "_", time_stamp, "_", feature, "_distributions.csv", sep = "")
  #  write.table(  all_distributions, file=table_path, sep=",", append = F, col.names=F, row.names=F )
  write.table(all_distributions, file = csvwell, sep = ",", append = T, col.names = F, row.names = F)
}
...
dist_perm <- function(datafile, np, type, kotype) {
  ...
  for (i in 1:np) {
    ...
    outEMD[i] <- emd(matrix(c(data_wt_original, 1:length(data_wt_original)), ncol = 2), matrix(c(data_ko_original, 1:length(data_wt_original)), ncol = 2))
  ...
  data_ko_original = colMeans(value[- wt_e, ])
  data_ko_original[is.na(data_ko_original)] = 0
  data_EMD <- emd(matrix(c(data_wt_original, 1:length(data_wt_original)), ncol = 2), matrix(c(data_ko_original, 1:length(data_wt_original)), ncol = 2))

  # compute permutaton p-value
  perm_p <- length(which(outp < data_p)) / np
  perm_EMD <- length(which(outEMD < data_EMD)) / np
  result <- list(data_EMD = data_EMD, data_p = data_p, perm_EMD = perm_EMD, perm_p = perm_p, outp = outp, outEMD = outEMD, data_wt = data_wt, data_ko = data_ko, data_wt_original = data_wt_original, data_ko_original = data_ko_original)
  return(result)
}
