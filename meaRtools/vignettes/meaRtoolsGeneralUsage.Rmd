...
r_object_dir<-paste0( data.dir , "/Analysis/R_Objects" )
...
analysis<-list(spikeFiles = spkListFiles, output.dir = output.dir, Routput.dir = r_object_dir, output.perDIV.dir = output.perDIV.dir)
...
for (i in 1:length(spkListFiles)){
  ...
  plate_chem_info<-get.experimental.log.file(spkListFiles[i], ExperimentalLogFile)
  
    # convert the spike list data to a 'spike.list' class Robject
  analysis$Robject[i]<-read_spikelist(key=title, spk_list_file=spkListFiles[i],          chem_info=plate_chem_info,r_object_dir=r_object_dir) 
}
```
...
plot_plate_summary_for_spikes(s,analysis$output.perDIV.dir)
...
# filter spike wells
spike.features = lapply(spike.features, function(x) filter.wells( x, nae, parameters$well_min_rate, parameters$well.filter.maximum.DIV.inactive.ratio))

# filter network burst wells
nb.features <- lapply(nb.features, function(x) filter.wells(x, nae, parameters$well_min_rate, parameters$well.filter.maximum.DIV.inactive.ratio ))
